...
// maior valor, menor valor, média e o desvio padrão das avaliações (campo imdb.rating).
...
// sequência de string parecida com essas abaixo,
...
db.movies.aggregate([
  { $match: {
    awards: { $regex: /^Won/i },
    $or: [{ awards: { $regex: /Oscar/i } }, { awards: { $regex: /Oscars/i } }],
  } },
  { $group: {
    _id: 0,
